...
Four entry points (CLI, planner, bot, watcher) share one fetch/parse core and integrate purely through a lock-guarded JSON file on disk — no database, no server-to-server calls.
...
Shipped and running. The watcher polls around the clock, notifications are live end-to-end, and the planner and shared bot are both in use. It reads only NC State's public catalog and stores no credentials — an unofficial personal tool with no affiliation to the university.
